...
const BASE_URL = process.env.NEXT_PUBLIC_CATALYST_API_BASE_URL || '/api/v1';
...
export async function getSocioInsights(params?: {
  ...
  const response = await apiClient.get('/insights/socio', { params });
  return response.data;
}

// ==================== Decision Support ====================

export async function getCaseSummary(caseId: string | number): Promise<any> {
  const response = await apiClient.get(`/support/case-summary/${caseId}`);
  return response.data;
}

export async function getSimilarCases(caseId: string | number): Promise<any> {
  const response = await apiClient.get(`/support/similar-cases/${caseId}`);
  return response.data;
}
...
export async function getTrendForecast(params?: {
  ...
  const response = await apiClient.get('/trends/forecast', { params });
  return response.data;
}

// ==================== Financial Crime ====================

export async function getFinancialLinks(accusedId: string | number): Promise<any> {
  const response = await apiClient.get(`/network/financial/${accusedId}`);
  return response.data;
}
