...
Reads environment variables from .env.backend (if present) using python-dotenv.
Catalyst-provided variables are NOT overwritten (override=False).
...
# ── Load local .env.backend (never overwrites already-set variables) ───────
# This runs BEFORE any imports that read os.getenv(), so all handlers pick up
# the correct values from the first line they execute.
try:
    from dotenv import load_dotenv
    # Resolve .env.backend relative to this script, not cwd
    _this_dir  = os.path.dirname(os.path.abspath(__file__)) if "__file__" in dir() else os.getcwd()
    _env_file  = os.path.join(_this_dir, ".env.backend")
...
# ── Safe defaults for local dev only (never overwrite already-set vars) ────
# These only apply if the variables are not already set (e.g. in CI or Docker).
...
# DB_PASSWORD has no default — must be set in .env.backend or system env
...
try:
    ...
    print(f"❌ Error importing handlers: {exc}")
    import traceback
    traceback.print_exc()
    sys.exit(1)

# ── OCR handler — optional (requires Zia SDK or torch+transformers) ─────────
try:
    ...
    OCR_AVAILABLE = False
    print(f"⚠️  OCR not available: {exc}")
...
def make_req(flask_req, path_params=None):
    """Convert a Flask request into a Catalyst-style request dict."""
    body = {}
    try:
...
def financial_links(accused_id):
    """Financial links — no transaction table yet; returns empty list."""
    return jsonify({
        "accused_id":   accused_id,
        "transactions": [],
        "note":         "Financial transaction data not available.",
    }), 200
...
def ingest_pending():
    try:
        ...
        print(f"❌ Ingest pending: {exc}")
        return jsonify({"error": str(exc)}), 500


@app.route("/api/v1/ingest/index", methods=["POST"])
def ingest_index():
    """Manual trigger for batch indexing (normally called by Catalyst Cron)."""
    try:
        return send(index_handler(make_req(request)))
    except Exception as exc:
        print(f"❌ Ingest index: {exc}")
        return jsonify({"error": str(exc)}), 500


# ── OCR ───────────────────────────────────────────────────────────────────────
def _ocr_file(file_obj):
    """Run OCR on an uploaded file object; returns extracted text or ''."""
    ...
    try:
        ...
        print(f"⚠️  OCR during upload failed: {exc}")
        return ""
    ...
    try:
        if request.content_type and "multipart" in request.content_type:
            ...
            if not moderation["safe"]:
                return jsonify({"error": "Image failed content moderation.", "moderation": moderation}), 422
            ...
            return jsonify({
                ...
                "ocr_engine": engine, "moderation": moderation,
...
# ── Startup banner ────────────────────────────────────────────────────────────
def _startup_banner():
    db_type   = "Catalyst Data Store" if os.getenv("USE_CATALYST_DS","false").lower() == "true" else "PostgreSQL"
    ...
    print("=" * 60)
    # Safety check: warn if password looks like it was never set
    if not os.getenv("DB_PASSWORD"):
        print("⚠️  DB_PASSWORD is not set — connections will fail unless PostgreSQL")
        print("   is configured to accept peer/trust auth for this user.")
    print()
